void Main()
{
	List<Password> passwords = new List<UserQuery.Password>();	
	
	foreach(var item in input.Split('\n'))			
		passwords.Add(new Password(item.Trim()));
	
	Console.WriteLine(passwords.Where(p => p.IsPartOneValid).Count());
	Console.WriteLine(passwords.Where(p => p.IsPartTwoValid).Count());	
}
...
public class Password
{
	...
	{
		var parts = raw.Split(' ');
		Min = Convert.ToInt32(parts[0].Split('-')[0]);
		Max = Convert.ToInt32(parts[0].Split('-')[1]);
		Letter = parts[1][0].ToString();
		Value = parts[2];
	}
	...
	public bool IsPartOneValid
	{
		get
		{
			var count = Value.ToCharArray().Count(v => v.ToString() == Letter);
	...
	public bool IsPartTwoValid
	{
		get
		{
			...
			if(chars[Min - 1].ToString() == Letter && chars[Max - 1].ToString() != Letter)
				return true;
...
string input = @"1-7 j: vrfjljjwbsv
1-10 j: jjjjjjjjjjjj
