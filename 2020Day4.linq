void Main()
{
	List<Passport> passports = new List<UserQuery.Passport>();	
	...
	for(int i = 0; i < lines.Length; i++)
	{
		...
		{
			if (string.IsNullOrWhiteSpace(currentPassport) == false)
	...
	passports.Where(t => t.IsPartOneValid()).Count().Dump();	
	passports.Where(t => t.IsPartTwoValid()).Count().Dump();	
}
...
public class Passport
{
	...
	public Passport(string data)
	{
		...
		string[] parts = data.Trim().Replace('\r', ' ').Split(' ');			
		
		foreach(var item in parts)
		{
			kvps.Add(item.Split(':')[0], item.Split(':')[1]);
		}
	}
	...
	public bool IsPartTwoValid()
	{
		if (IsPartOneValid())
		{
			if (Int32.TryParse(kvps["byr"], out int val))
			{
			...
			if (Int32.TryParse(kvps["iyr"], out int iyrval))
			{
			...
			if (Int32.TryParse(kvps["eyr"], out int eyr))
			{
			...
			if (kvps["hgt"].EndsWith("in") == false && kvps["hgt"].EndsWith("cm") == false)
				return false;

			var height = Convert.ToInt32(kvps["hgt"].Trim('c', 'm', 'i', 'n'));
			bool isCm = kvps["hgt"].EndsWith("cm");

			if (isCm && (height < 150 || height > 193))
				return false;
			else if (isCm == false && (height < 59 || height > 76))
				return false;

			if (kvps["hcl"].Length != 7 || kvps["hcl"][0] != '#' || System.Text.RegularExpressions.Regex.IsMatch(kvps["hcl"].Substring(1), @"\A\b[0-9a-fA-F]+\b\Z") == false)			
				return false;
			
			if ("amb blu brn gry grn hzl oth".Contains(kvps["ecl"].ToLower()) == false)
				return false;

			if (kvps["pid"].Length != 9 || Int32.TryParse(kvps["pid"], out int t) == false)
				return false;
	...
	public bool IsPartOneValid()
	{
		if (kvps.ContainsKey("byr") &&
			kvps.ContainsKey("iyr") &&
			kvps.ContainsKey("eyr") &&
			kvps.ContainsKey("hgt") &&
			kvps.ContainsKey("hcl") &&
			kvps.ContainsKey("ecl") &&
			kvps.ContainsKey("pid"))
			{						
			return true;
		}
