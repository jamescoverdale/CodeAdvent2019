...
def didCollide(loc1, loc2) :
    return set(loc1) & set(loc2)
...
def parse(move, loc) :
    ...
    if move[0] == 'U' :       
        ...
        for i in range(1,y + 1) :
            movements.append((loc[0], loc[1] + i))
        ...
        for i in range(1,y + 1) :
            movements.append((loc[0] - i, loc[1]))
    elif move[0] == 'R' :
        for i in range(1,y + 1) :
            movements.append((loc[0] + i, loc[1]))  
...
for index in range(0, max(len(wire1), len(wire2))) :
    
    if index < len(wire1) : 
        wire1Loc.extend(parse(wire1[index], wire1Loc[-1]))
        #print(wire1Loc)

    if index < len(wire2) : 
        wire2Loc.extend(parse(wire2[index], wire2Loc[-1]))
        #print(wire2Loc)
      
    index += 1

cols = didCollide(wire1Loc, wire2Loc)
lowest = 999999

for xx in cols :

    oneIndex = wire1Loc.index(xx)
    twoIndex = wire2Loc.index(xx)

    if (oneIndex + twoIndex) < lowest and (oneIndex + twoIndex) != 0 :
        lowest = (oneIndex + twoIndex)

print(lowest)
# graph it
# plt.scatter(*zip(*wire1Loc))
# plt.scatter(*zip(*wire2Loc))
# plt.show()